...
// Open or create an IndexedDB with the name medicinesDB
function openDB() {
    let request = indexedDB.open('medicinesDB', 1); 

    request.onupgradeneeded = function(event) {
        db = event.target.result;

        // Create medicines object store if it doesn't exist
        if (!db.objectStoreNames.contains('medicines')) {
            let objectStore = db.createObjectStore('medicines', { keyPath: 'id', autoIncrement: true });
        }
    ...
    request.onsuccess = function(event) {
        ...
        fetchAndStoremedicines();  // call and run function
        populatemedicineselect();  // call and run function to populate data in select dropdown
    };
...
function addPatient() {
    ...
    // Get all medicines and find the next available ID
    let request = objectStore.getAll();

    request.onsuccess = function(event) {
        let allmedicines = event.target.result;
        let nextId = allmedicines.length > 0 ? allmedicines[allmedicines.length - 1].id + 1 : 1;
...
// Fetch medicines data from the JSON URL and store it in IndexedDB
...
function loadPatientData() {
    const patientId = document.getElementById('medicineselect').value;
...
function populatemedicineselect() {
    ...
    const request = objectStore.getAll(); // Get all medicines

    request.onsuccess = function(event) {
        const medicines = event.target.result;
        medicines.forEach(patient => {
            ...
            medicineselect.appendChild(option); // Add option to the select element
        });
    };
...
function createPatientLoginDetails() {
    ...
    let patientId = document.getElementById('medicineselect').value; 
    let username = document.getElementById('username').value;
    let password = document.getElementById('password').value;

    // Begin a transaction on both the medicines and userLogins object stores
    let transaction = db.transaction(['medicines', 'userLogins'], 'readwrite');
    let medicinestore = transaction.objectStore('medicines');
    let loginStore = transaction.objectStore('userLogins');

    // Retrieve the patient based on the selected ID
    let request = medicinestore.get(parseInt(patientId));
